...
class ForgeMainWindow(QMainWindow):
    ...
    def setup_engine(self, session):
        ...
        # Set git panel to the session's working directory
        import os
        cwd = os.getcwd()
        if hasattr(session, 'path') and session.path:
            cwd = session.path[0]
    ...
    def _connect_signals(self):
        ...
        # Update git panel when the file browser path changes
        self.file_browser_widget.path_changed.connect(
            lambda paths: self.git_widget.set_repo_path(paths[0]) if paths else None
        )
    ...
    def _on_git_status(self, message):
        """Display a git panel status message in the status bar."""
    ...
    def _create_actions(self):
        ...
        self.action_toggle_git = QAction("Git", self, checkable=True, checked=True)
    ...
    def _create_docks(self):
        ...
        from forge.gui.git_panel import GitPanelWidget
        ...
        # --- Git Panel (left, below file browser) ---
        self.git_widget = GitPanelWidget(self)
        self.git_dock = self._make_dock("Git", "GitDock", self.git_widget)
        self.addDockWidget(Qt.LeftDockWidgetArea, self.git_dock)
        self.action_toggle_git.toggled.connect(self.git_dock.setVisible)
        self.git_dock.visibilityChanged.connect(self.action_toggle_git.setChecked)
        self.git_widget.status_message.connect(self._on_git_status)
