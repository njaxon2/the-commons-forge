...
def forge_sparse(i_or_m, j=None, v=None, m=None, n=None):
    """Create sparse matrix.  1-arg: dense->sparse.  3+ args: COO triplets."""
    mat = _unwrap(i_or_m)
    if j is None:
        # dense -> sparse
        return sp.csc_matrix(mat)
    i_arr = np.asarray(_unwrap(mat), dtype=int).ravel() - 1  # 1-based -> 0-based
    j_arr = np.asarray(_unwrap(j), dtype=int).ravel() - 1
    v_arr = np.asarray(_unwrap(v), dtype=float).ravel()
    if m is None:
        m = int(i_arr.max()) + 1
    if n is None:
        n = int(j_arr.max()) + 1
