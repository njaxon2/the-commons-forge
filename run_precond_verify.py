#!/usr/bin/env python3
"""Verify diagonal preconditioning across mesh sizes and arc angles."""
import sys, os, time
import numpy as np
...
def build_and_precondition(nel, R1, R2, w_mid, theta):
    """Build K, apply diagonal preconditioning, return both condition numbers."""
...
print("=== Preconditioning Verification ===\n", flush=True)

# Part 1: Different mesh sizes
R1, R2 = 0.5, 1.5
theta = np.pi/2
w_geo = 1/np.sqrt(2)

print("Part 1: Mesh independence of preconditioning", flush=True)
print(f"{'nel':>4} {'cond_geo':>12} {'cond_prec':>12} {'cond_bsp':>12} {'geo/bsp':>8} {'prec/bsp':>9}", flush=True)
for nel in [2, 4, 8]:
    c_geo, c_prec = build_and_precondition(nel, R1, R2, w_geo, theta)
    s.eval(f'[_cb,_,_,_] = compute_annulus_cond(2, {nel}, 4, {R1}, {R2}, 1.0);')
    c_bsp = float(s.eval('_cb').strip())
    print(f"{nel:4d} {c_geo:12.4e} {c_prec:12.4e} {c_bsp:12.4e} {c_geo/c_bsp:8.4f} {c_prec/c_bsp:9.4f}", flush=True)

# Part 2: Different arc angles
print(f"\nPart 2: Arc angle independence", flush=True)
print(f"{'theta':>6} {'cond_geo':>12} {'cond_prec':>12} {'cond_bsp':>12} {'geo/bsp':>8} {'prec/bsp':>9}", flush=True)
nel = 4
for theta_d in [30, 60, 90, 120]:
    theta_r = theta_d * np.pi / 180
    w_g = np.cos(theta_r/2)
    c_geo, c_prec = build_and_precondition(nel, R1, R2, w_g, theta_r)
    # B-spline at same geometry
    c_geo_bsp, _ = build_and_precondition(nel, R1, R2, 1.0, theta_r)
    print(f"{theta_d:6d} {c_geo:12.4e} {c_prec:12.4e} {c_geo_bsp:12.4e} {c_geo/c_geo_bsp:8.4f} {c_prec/c_geo_bsp:9.4f}", flush=True)
