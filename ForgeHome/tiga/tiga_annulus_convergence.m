%% TIGA h-Convergence on Quarter Annulus (Mapped Geometry)
%  Solve -Lap(u) = f on quarter annulus with manufactured solution
%  u = (r^2 - R1^2)*(R2^2 - r^2), homogeneous BCs on inner/outer
%  Compute L2 and Linf errors for multiple mesh sizes, plot log-log
%  Expected: rate p+1 in L2 for smooth solution

clear;
fprintf('=== TIGA Annulus h-Convergence Study ===\n\n');
...
h_vals = zeros(1, n_trials);
err_linf = zeros(1, n_trials);
err_l2 = zeros(1, n_trials);
...
for trial = 1:n_trials
    ...
    % Evaluate error on plotting grid
    n_plot = 40;
    ...
    err_linf(trial) = e_max;
    err_l2(trial) = sqrt(e_l2_sum / area_sum);
    fprintf('  nel=%2d  DOF=%3d  Linf=%.4e  L2=%.4e\n', nel_r, n_2d, e_max, err_l2(trial));
end

% Compute convergence rates
fprintf('\n  h-convergence rates:\n');
for i = 2:n_trials
    rate_linf = log(err_linf(i-1) / err_linf(i)) / log(h_vals(i-1) / h_vals(i));
    rate_l2 = log(err_l2(i-1) / err_l2(i)) / log(h_vals(i-1) / h_vals(i));
    fprintf('    h=1/%d -> 1/%d:  Linf rate=%.2f  L2 rate=%.2f\n', nel_list(i-1), nel_list(i), rate_linf, rate_l2);
end

% Log-log convergence plot
figure(1);
loglog(h_vals, err_linf, 'bo-');
hold on;
loglog(h_vals, err_l2, 'rs-');

% Reference slopes
h_ref = [h_vals(1), h_vals(end)];
c_linf = err_linf(1) / h_vals(1)^(p+1);
c_l2 = err_l2(1) / h_vals(1)^(p+1);
loglog(h_ref, c_linf * h_ref.^(p+1), 'b--');
loglog(h_ref, c_l2 * h_ref.^(p+1), 'r--');

xlabel('h (element size)');
ylabel('Error');
title(sprintf('h-Convergence on Quarter Annulus (p=%d)', p));
legend('L_inf', 'L_2', sprintf('O(h^%d) ref', p+1), sprintf('O(h^%d) ref', p+1));
hold off;
drawnow;

fprintf('\n=== Annulus h-Convergence Complete ===\n');
