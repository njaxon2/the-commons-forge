%% TIGA: Systematic NURBS Weight Sweep
%  Sweep the middle control point weight from 0.01 to 10
%  and measure condition number, eigenvalue spectrum, and solution error
%  Goal: find the relationship cond(K) = f(w) analytically

clear;
fprintf('=== TIGA Weight Sweep Study ===\n\n');
...
w_list = [0.01, 0.05, 0.1, 0.2, 0.3, 0.5, 1/sqrt(2), 1.0, 1.5, 2.0, 3.0, 5.0, 10.0];
n_w = length(w_list);
cond_vals = zeros(1, n_w);
lmin_vals = zeros(1, n_w);
lmax_vals = zeros(1, n_w);
err_vals = zeros(1, n_w);
...
for iw = 1:n_w
    ...
    cond_vals(iw) = max(ev_pos) / min(ev_pos);
    lmin_vals(iw) = min(ev_pos);
    lmax_vals(iw) = max(ev_pos);
    ...
    fprintf('  w=%.4f  cond=%.4e  lmin=%.4e  lmax=%.4e  err=%.4e\n', ...
        w_mid, cond_vals(iw), lmin_vals(iw), lmax_vals(iw), err_vals(iw));
end

%% Plots
figure(1);
subplot(2, 2, 1);
loglog(w_list, cond_vals, 'bo-');
xlabel('Weight w');
ylabel('cond(K)');
title('Condition Number vs Weight');

subplot(2, 2, 2);
loglog(w_list, lmin_vals, 'rs-');
hold on;
loglog(w_list, lmax_vals, 'bd-');
hold off;
xlabel('Weight w');
ylabel('Eigenvalue');
title('Extreme Eigenvalues vs Weight');
legend('lambda_{min}', 'lambda_{max}');

subplot(2, 2, 3);
loglog(w_list, err_vals, 'gd-');
xlabel('Weight w');
ylabel('Max Error');
title('Solution Error vs Weight');

subplot(2, 2, 4);
% Normalized condition: cond(w) / cond(w=1)
cond_w1 = cond_vals(w_list == 1.0);
if length(cond_w1) == 0
    cond_w1 = cond_vals(8);
end
semilogx(w_list, cond_vals / cond_w1, 'ko-');
xlabel('Weight w');
ylabel('cond(K) / cond(K)|_{w=1}');
title('Normalized Condition Number');
