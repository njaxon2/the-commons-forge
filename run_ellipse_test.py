#!/usr/bin/env python3
"""Test weight-conditioning tradeoff on an elliptical annulus.

For an ellipse with semi-axes a, b: the NURBS weight for a quarter
ellipse is still w = cos(theta/2) where theta = pi/2, so w = 1/sqrt(2).
But the control points change: P0=(a,0), P1=(a,b), P2=(0,b) with w1=1/sqrt(2).

Question: does the diagonal preconditioning still work?
"""
import sys, os, time
import numpy as np
sys.path.insert(0, '/home/ubuntu/forge')
os.environ.setdefault('DISPLAY', ':99')

from forge.engine.session import ForgeSession
s = ForgeSession()
s.eval('addpath("ForgeHome/tiga")')

def build_ellipse_system(nel, a_in, b_in, a_out, b_out, w_mid):
    """Build stiffness matrix for quarter elliptical annulus.

    Inner ellipse: semi-axes (a_in, b_in)
    Outer ellipse: semi-axes (a_out, b_out)
    """
    s.eval(f'_p=2; _nel_r={nel}; _nqp=4; _w={w_mid};')
    s.eval(f'_ai={a_in}; _bi={b_in}; _ao={a_out}; _bo={b_out};')

    s.eval("""_interior_r = linspace(0, 1, _nel_r + 1);
_interior_r = _interior_r(2:end-1);
_Xi_r = [zeros(1, _p+1), _interior_r, ones(1, _p+1)];
_n_r = length(_Xi_r) - _p - 1;
_Xi_t = [0 0 0 1 1 1];
_n_t = 3;
_n_2d = _n_r * _n_t;""")

    # Control points for elliptical annulus
    # Each radial station interpolates between inner and outer ellipse
    s.eval("""_r_frac = linspace(0, 1, _n_r);
_CPx = zeros(_n_r, _n_t); _CPy = zeros(_n_r, _n_t); _Cw = ones(_n_r, _n_t);
for _i = 1:_n_r
    _t = _r_frac(_i);
    _ax = _ai + _t * (_ao - _ai);
    _bx = _bi + _t * (_bo - _bi);
    _CPx(_i,1) = _ax; _CPy(_i,1) = 0;   _Cw(_i,1) = 1;
    _CPx(_i,2) = _ax; _CPy(_i,2) = _bx;  _Cw(_i,2) = _w;
    _CPx(_i,3) = 0;   _CPy(_i,3) = _bx;  _Cw(_i,3) = 1;
end""")

    s.eval("""[_gp, _gw] = gaussQuad(_nqp);
_knots_r = unique(_Xi_r);
_knots_t = unique(_Xi_t);
_nel_rad = length(_knots_r) - 1;
_nel_cir = length(_knots_t) - 1;""")

    assembly = """_K = zeros(_n_2d, _n_2d);
for _er = 1:_nel_rad
    _xi_a=_knots_r(_er); _xi_b=_knots_r(_er+1);
    if _xi_b-_xi_a<1e-14; continue; end
    _Jr=(_xi_b-_xi_a)/2;
    for _et = 1:_nel_cir
        _eta_a=_knots_t(_et); _eta_b=_knots_t(_et+1);
        if _eta_b-_eta_a<1e-14; continue; end
        _Jt=(_eta_b-_eta_a)/2;
        for _qr = 1:_nqp
            _xi=(_xi_a+_xi_b)/2+_Jr*_gp(_qr);
            _span_r=findspan(_n_r-1,_p,_xi,_Xi_r);
            _ders_r=derbasisfun(_span_r,_xi,_p,1,_Xi_r);
            _Nr=_ders_r(1,:); _dNr=_ders_r(2,:);
            for _qt = 1:_nqp
                _eta=(_eta_a+_eta_b)/2+_Jt*_gp(_qt);
                _span_t=findspan(_n_t-1,_p,_eta,_Xi_t);
                _ders_t=derbasisfun(_span_t,_eta,_p,1,_Xi_t);
                _Nt=_ders_t(1,:); _dNt=_ders_t(2,:);
                _wt_q=_gw(_qr)*_Jr*_gw(_qt)*_Jt;
                _W=0; _dW_dxi=0; _dW_deta=0;
                for _a=0:_p
                    _ir=_span_r-_p+_a+1;
                    for _b=0:_p
                        _it=_span_t-_p+_b+1;
                        _wv=_Cw(_ir,_it);
                        _W=_W+_Nr(_a+1)*_Nt(_b+1)*_wv;
                        _dW_dxi=_dW_dxi+_dNr(_a+1)*_Nt(_b+1)*_wv;
                        _dW_deta=_dW_deta+_Nr(_a+1)*_dNt(_b+1)*_wv;
                    end
                end
                _dx_dxi=0;_dx_deta=0;_dy_dxi=0;_dy_deta=0;
                for _a=0:_p
                    _ir=_span_r-_p+_a+1;
                    for _b=0:_p
                        _it=_span_t-_p+_b+1;
                        _wv=_Cw(_ir,_it);
                        _dR_dxi=(_dNr(_a+1)*_Nt(_b+1)*_wv*_W-_Nr(_a+1)*_Nt(_b+1)*_wv*_dW_dxi)/_W^2;
                        _dR_deta=(_Nr(_a+1)*_dNt(_b+1)*_wv*_W-_Nr(_a+1)*_Nt(_b+1)*_wv*_dW_deta)/_W^2;
                        _dx_dxi=_dx_dxi+_dR_dxi*_CPx(_ir,_it);
                        _dx_deta=_dx_deta+_dR_deta*_CPx(_ir,_it);
                        _dy_dxi=_dy_dxi+_dR_dxi*_CPy(_ir,_it);
                        _dy_deta=_dy_deta+_dR_deta*_CPy(_ir,_it);
                    end
                end
                _detJ=_dx_dxi*_dy_deta-_dx_deta*_dy_dxi;
                if abs(_detJ)<1e-15; continue; end
                _inv_J11=_dy_deta/_detJ; _inv_J12=-_dy_dxi/_detJ;
                _inv_J21=-_dx_deta/_detJ; _inv_J22=_dx_dxi/_detJ;
                for _a=0:_p
                    _ir=_span_r-_p+_a+1;
                    for _b=0:_p
                        _it=_span_t-_p+_b+1;
                        _wA=_Cw(_ir,_it);
                        _dRA_dxi=(_dNr(_a+1)*_Nt(_b+1)*_wA*_W-_Nr(_a+1)*_Nt(_b+1)*_wA*_dW_dxi)/_W^2;
                        _dRA_deta=(_Nr(_a+1)*_dNt(_b+1)*_wA*_W-_Nr(_a+1)*_Nt(_b+1)*_wA*_dW_deta)/_W^2;
                        _dRA_dx=_inv_J11*_dRA_dxi+_inv_J12*_dRA_deta;
                        _dRA_dy=_inv_J21*_dRA_dxi+_inv_J22*_dRA_deta;
                        _gA=(_it-1)*_n_r+_ir;
                        for _c=0:_p
                            _jr=_span_r-_p+_c+1;
                            for _d=0:_p
                                _jt=_span_t-_p+_d+1;
                                _wB=_Cw(_jr,_jt);
                                _dRB_dxi=(_dNr(_c+1)*_Nt(_d+1)*_wB*_W-_Nr(_c+1)*_Nt(_d+1)*_wB*_dW_dxi)/_W^2;
                                _dRB_deta=(_Nr(_c+1)*_dNt(_d+1)*_wB*_W-_Nr(_c+1)*_Nt(_d+1)*_wB*_dW_deta)/_W^2;
                                _dRB_dx=_inv_J11*_dRB_dxi+_inv_J12*_dRB_deta;
                                _dRB_dy=_inv_J21*_dRB_dxi+_inv_J22*_dRB_deta;
                                _gB=(_jt-1)*_n_r+_jr;
                                _K(_gA,_gB)=_K(_gA,_gB)+(_dRA_dx*_dRB_dx+_dRA_dy*_dRB_dy)*abs(_detJ)*_wt_q;
                            end
                        end
                    end
                end
            end
        end
    end
end"""
    s.eval(assembly)

    # BCs and condition number
    s.eval("""_bc=[];
for _j=1:_n_t
    _bc=[_bc,(_j-1)*_n_r+1,(_j-1)*_n_r+_n_r];
end
_bc=unique(_bc);
_free=setdiff(1:_n_2d,_bc);
_Kf=_K(_free,_free);
_ev=sort(real(eig(_Kf)));
_evp=_ev(_ev>1e-10);
_cond_orig=max(_evp)/min(_evp);""")
    cond_orig = float(s.eval('_cond_orig').strip())

    # Diagonal preconditioning
    s.eval("_D=diag(_Kf); _Dinv=diag(1./sqrt(_D)); _Kp=_Dinv*_Kf*_Dinv;")
    s.eval("_evp2=sort(real(eig(_Kp))); _evp2=_evp2(_evp2>1e-10);")
    s.eval("_cond_prec=max(_evp2)/min(_evp2);")
    cond_prec = float(s.eval('_cond_prec').strip())

    return cond_orig, cond_prec

print("=== Elliptical Annulus: Weight-Conditioning Test ===\n", flush=True)

nel = 4
w_geo = 1/np.sqrt(2)

# Test 1: Circle (baseline, should match previous results)
print("Circle (a=b): R_in=0.5, R_out=1.5", flush=True)
c_geo, c_prec = build_ellipse_system(nel, 0.5, 0.5, 1.5, 1.5, w_geo)
c_bsp, _ = build_ellipse_system(nel, 0.5, 0.5, 1.5, 1.5, 1.0)
print(f"  cond_geo={c_geo:.2f}, cond_bsp={c_bsp:.2f}, cond_prec={c_prec:.2f}")
print(f"  geo/bsp={c_geo/c_bsp:.4f}, prec/bsp={c_prec/c_bsp:.4f}", flush=True)

# Test 2: Mild ellipse (a/b = 2)
print("\nMild ellipse (a/b=2): a_in=0.5,b_in=0.25, a_out=1.5,b_out=0.75", flush=True)
c_geo, c_prec = build_ellipse_system(nel, 0.5, 0.25, 1.5, 0.75, w_geo)
c_bsp, _ = build_ellipse_system(nel, 0.5, 0.25, 1.5, 0.75, 1.0)
print(f"  cond_geo={c_geo:.2f}, cond_bsp={c_bsp:.2f}, cond_prec={c_prec:.2f}")
print(f"  geo/bsp={c_geo/c_bsp:.4f}, prec/bsp={c_prec/c_bsp:.4f}", flush=True)

# Test 3: Strong ellipse (a/b = 4)
print("\nStrong ellipse (a/b=4): a_in=1.0,b_in=0.25, a_out=2.0,b_out=0.5", flush=True)
c_geo, c_prec = build_ellipse_system(nel, 1.0, 0.25, 2.0, 0.5, w_geo)
c_bsp, _ = build_ellipse_system(nel, 1.0, 0.25, 2.0, 0.5, 1.0)
print(f"  cond_geo={c_geo:.2f}, cond_bsp={c_bsp:.2f}, cond_prec={c_prec:.2f}")
print(f"  geo/bsp={c_geo/c_bsp:.4f}, prec/bsp={c_prec/c_bsp:.4f}", flush=True)

# Test 4: Non-conformal (different aspect ratios inner vs outer)
print("\nNon-conformal: a_in=0.5,b_in=0.5, a_out=2.0,b_out=1.0", flush=True)
c_geo, c_prec = build_ellipse_system(nel, 0.5, 0.5, 2.0, 1.0, w_geo)
c_bsp, _ = build_ellipse_system(nel, 0.5, 0.5, 2.0, 1.0, 1.0)
print(f"  cond_geo={c_geo:.2f}, cond_bsp={c_bsp:.2f}, cond_prec={c_prec:.2f}")
print(f"  geo/bsp={c_geo/c_bsp:.4f}, prec/bsp={c_prec/c_bsp:.4f}", flush=True)

print("\n=== Ellipse Test Complete ===", flush=True)
